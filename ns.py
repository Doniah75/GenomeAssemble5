...
def generate_plots(results):
    ...
    for metric_name, (free_values, prone_values) in metrics.items():
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f"plots/{metric_name.replace(' ', '_').lower()}_vs_N.png")
        plt.close()
...
def run_full_experiments(genome):
    Ns = [50, 100, 200, 300, 500, 750, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000]
    params = [('N', (genome, N, 100, 0.1, 20)) for N in Ns]
