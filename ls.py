...
def generate_plots(results):
    ...
    for metric_name, (free_values, prone_values) in metrics.items():
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f"plots/{metric_name.replace(' ', '_').lower()}_vs_l.png")
        plt.close()
...
def run_full_experiments(genome):
    ls = [10, 25, 40, 50, 60, 75, 90, 100, 125, 150, 175, 200, 225, 250, 275, 300, 350, 400, 450, 500]
    params = [('l', (genome, 1000, l, 0.1, 20)) for l in ls]

    with Pool(processes=7) as pool:
        results = pool.map(run_experiment_wrapper, [x[1] for x in params])

    results = [(params[i][0], params[i][1][2], res) for i, res in enumerate(results)]
