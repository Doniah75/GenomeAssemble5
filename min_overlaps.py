...
def generate_plots(results):
    ...
    for metric_name, (free_values, prone_values) in metrics.items():
        ...
        plt.legend()
        plt.grid()
        plt.savefig(f"plots/{metric_name.replace(' ', '_').lower()}_vs_min_overlap.png")
        plt.close()
...
def run_full_experiments(genome):
    min_overlaps = [1, 5, 7, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100]
    params = [('min_overlap', (genome, 1000, 100, 0.1, min_overlap)) for min_overlap in min_overlaps]

    with Pool(processes=7) as pool:
        results = pool.map(run_experiment_wrapper, [x[1] for x in params])

    results = [(params[i][0], params[i][1][4], res) for i, res in enumerate(results)]
